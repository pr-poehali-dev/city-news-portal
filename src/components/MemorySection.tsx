...
export function MemorySection({ articles, onArticleClick }: MemorySectionProps) {
  ...
  const otherArticles = publishedArticles.slice(1, 4);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card 
          ...
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
            onArticleClick(latestArticle.id);
          }}
          style={{
            ...
            {latestArticle.image_url && (
              <div className="relative h-80 overflow-hidden">
                ...
                <div className="absolute top-4 left-4 bg-black/70 backdrop-blur-sm px-6 py-3 border-2 border-amber-400/80">
                  <span className="text-3xl font-serif font-bold text-amber-100 tracking-wider">{latestArticle.year}</span>
                </div>
              </div>
            )}
            ...
            <div className="p-6 bg-gradient-to-b from-transparent to-amber-50/30 dark:to-amber-950/20 flex-1 flex flex-col">
              <div className="flex items-center gap-2 mb-4 pb-2 border-b border-amber-300/30">
                ...
                <span className="text-xs text-amber-800 dark:text-amber-400 font-serif uppercase tracking-widest">
                  {latestArticle.decade || `${latestArticle.year}-е годы`}
                </span>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-amber-300/30">
                ...
                <Button 
                  variant="ghost" 
                  size="sm" 
                  className="text-amber-800 hover:text-amber-900 dark:text-amber-400 dark:hover:text-amber-300 font-serif"
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    onArticleClick(latestArticle.id);
              ...
              <CardContent className="p-0 relative">
                <div className="flex gap-4">
                  ...
                  <div className="p-4 flex-1">
                    ...
                    <h3 className="text-base font-serif font-bold mb-2 leading-tight text-amber-950 dark:text-amber-100 group-hover:text-amber-800 dark:group-hover:text-amber-300 transition-colors line-clamp-2">
                      {article.title}
                    </h3>
